...
export class RoleService {
  ...
  updateRole(useEditId, _id: Types.ObjectId, data: Role): Promise<Role> {
    return this.roleModel
      .findOneAndUpdate({ _id, creatorId: { $in: useEditId } }, data, {
        new: true,
      })
      .then(role => {
  ...
  async paginateList(querys: any, options: any): Promise<any> {
    ...
    const paginate = await (this.roleModel as any).aggregatePaginate(
      roles,
      options,
    );
      ...
      .then(role => {
        if (role) {
          return Promise.all([
            ...
            this.roleMenuModel.insertMany(
              data.menuIds.map(_id => {
                return {
                  roleId: data.roleId,
                  menuId: _id,
                };
